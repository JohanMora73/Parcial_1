...
int main()
{
    ...
    cout<<endl<<"ingrese la altura del canon ofensivo: "<<endl;cin>>Ho;
    cout<<"ingrese la distancia que separa los canones: "<<endl;cin>>d;
    cout<<"ingrese la altura del canon defensivo: "<<endl;cin>>Hd;
    yo0=Ho;
    ...
    switch(caso){
        ...
        case 3:
            cout<<"ingrese los datos del disparo del caño ofensivo (angulo en grados y velocidad respectivamente): "<<endl;
            cin>>ao>>vo;
            ao=ao*pi/180;
            vox=vo*cos(ao);
            voy=vo*sin(ao);
            ...
            if(riezgo==true){
                cout<<"El disparo ofensivo representa un riezgo: "<<endl;
            while(cont<3){
                ad = rand () % (90-0+1) + 0;
                ad=ad*pi/180;
                for(int vd = 0;vd<100;vd+=2){
        ...
        //Dado un disparo ofensivo genera tres posibles disparos defensivos que no comprometen al adversario
        case 4:
                ...
                for(int vd = 0;vd<100;vd+=2){
                    ...
                    for(int t =0;t<20;t+=1){
                        ...
                        if(sqrt(pow((x-xo),2)+pow((y-yo),2))<=0.025*d && sqrt(pow((xo-d),2)+pow((yo-Hd),2))>0.05*d && sqrt(pow((x-0),2)+pow((y-Ho),2))>0.025*d){
                            cout<<"Un disparo defensivo y ceretero que no compromete la integridad del cañon adversario es: angulo"<<ad*180/pi<<", velovidad: "<<vd<<" tiempo de vuelo: "<<t<<endl;
                            cont++;
                            resp=true;
                            break;
                        }
